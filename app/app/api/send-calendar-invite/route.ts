...
export async function POST(request: Request) {
  try {
    ...
    // 1. First, create the iCal event BEFORE sending the email
    ...
    try {
      ...
      // Parse duration (handle number, 'HH:MM' and 'X min' formats)
      let durationInMinutes = 30; // default
      const durationStr = String(body.duration);

      if (durationStr.includes(":")) {
        const [hours, minutes] = durationStr.split(":").map(Number);
        durationInMinutes = hours * 60 + minutes;
      } else {
        // Handle 'X min' format or just plain number
      ...
      // Build attendees array - only include attendees with valid emails
      const attendees = [];

      // Always add the recipient (they're the main attendee for this event)
      if (body.recipientType === "patient") {
        attendees.push({
      ...
      const eventData = {
        ...
        description:
          `Appointment with Dr. ${body.doctorName} for ${body.type} consultation\n\n` +
        ...
        location:
          body.type === "video" || body.type === "extended_video"
            ? body.meetingLink || "Online"
            : "In-Person",
      ...
      if (error) {
        ...
        // Continue without iCal event instead of failing
      } else {
        icsEvent = value;
        console.log("iCal event created successfully");
      }
    } catch (icsError) {
      console.error("Error creating iCal event:", icsError);
      // Continue without iCal event instead of failing
    }

    // 2. Send the email confirmation WITH the iCal attachment
    try {
